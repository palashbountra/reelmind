...
import { Search, Grid3X3, List, Plus, CheckSquare, X, Trash2, Wand2 } from "lucide-react";
...
export function DashboardView() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.category && filters.category !== "all") {
      // Match primary category OR extra_categories (non-destructive multi-category)
      list = list.filter(
        (r) =>
          r.category === filters.category ||
          (r.extra_categories ?? []).includes(filters.category as string)
      );
    }
              ...
              <div className="flex items-center gap-2">
                {selectMode ? (
                  ...
                  <>
                    ...
                    <button
                      onClick={() => setAutoOrganiserOpen(true)}
                      className="flex items-center gap-2 px-3 py-2 bg-violet-500/10 border border-violet-500/30 text-violet-300 text-sm font-medium rounded-xl hover:bg-violet-500/20 transition-all"
                      title="Auto-organise reels by buzzwords"
                    >
                      <Wand2 size={14} /> Auto-Organise
                    </button>
                    <button
